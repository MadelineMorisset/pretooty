<!DOCTYPE html>
<html lang="fr-FR">
    <head>
        <meta charset="UTF-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <!-- CSS -->
        <link rel="stylesheet" href="assets/css/style.css">
        <!-- Google Fonts-->
        <link rel="preconnect" href="https://fonts.googleapis.com">
        <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
        <link href="https://fonts.googleapis.com/css2?family=Lora&family=Merriweather:wght@700&display=swap" rel="stylesheet"><!-- Titles' Font : "Merriweather" / Texts' Font : "Lora" -->
        <!-- FontAwesome -->
        <script src="https://kit.fontawesome.com/23c1a897ea.js" crossorigin="anonymous"></script>
        <!-- JavaScript -->
        <script src="assets/js/script.js"></script>

        <title>Pretooty</title>
        <meta name="description" content="">
    </head>

    <body>
        <?php include("views/header.php"); 
        include('php/functions.php'); ?>

        <main>
            <h1 class="title titleWelcome">Bienvenue sur Pretooty</h1>

            <section class="indexBox">
                <section class="indexInnerBox">
                    <h2 class="title title24px">Qu’est-ce que “Pretooty” ?</h2>
                    <p class="text fontSize18px">
                        C’est une plateforme sur laquelle chacun peut prêter et/ou emprunter à d’autres utilisateurs des outils sectioners (jardinage, manutention, peinture, ...).
                    </p>
                </section>
                <section class="indexInnerBox">
                    <h2 class="title title24px">Qui peut prêter / emprunter les outils sur Pretooty ?</h2>
                    <p class="text fontSize18px">
                        Tous les utilisateurs, c’est à dire, tous ceux s’étant inscris sur la plateforme.
                        De plus, chaque utilisateur peut être prêteur ET emprunteur, cela sur un même compte (voir les sections “Prêteur” et “Emprunteur” dans les informations utilisateur).
                    </p>
                </section>
                <section class="indexInnerBox">
                    <h2 class="title title24px">Comment utiliser Pretooty ?</h2>
                    <p class="text fontSize18px">
                        (À venir ...)
                    </p>
                </section>
            </section>

            <section>
                <?php showAllTools(); ?>
            </section>
        </main>
        
        <?php include("views/footer.php"); ?>
    </body>
</html>